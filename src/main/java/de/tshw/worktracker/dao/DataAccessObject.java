 ...
 *
 * <p>Inherit from this class to create a straight-forwards DAO to handle basic read and write
...
public abstract class DataAccessObject<T extends Entity> {
     ...
     *
     * <p>Usually used as <code>getDbProvider().getDatabaseConnection();</code> right before
     ...
     *
     * <p>Note: The class mostly deals with caching itself.
     *
     ...
     * @return List of all entities in the database, sorted by the default sorting defined in {@link DataAccessObject#getDefaultOrderBy()}
     */
    public List<T> getAll() {
        ...
        try {
            ...
            System.err.println("Failed to retrieve entities from database. (" + ex.getClass().getName() + ": " + ex.getMessage() + ")");
        }
        return entities;
    }
    ...
    public T findById(long id) {
        ...
        if (!this.isEntityCached(id)) {
            try {
                ...
                System.err.println("Failed to retrieve entity from database. (" + ex.getClass().getName() + ": " + ex.getMessage() + ")");
            }
        } else {
     ...
     *
     * <p>You can call this with a new or an already known entity. If it isn't in the database yet, it will
        ...
        try {
            ...
            if (entity.getId() == null) {
                ...
                if ((result != null) && result.next()) {
                    ...
                    System.err.println("Failed to retrieve auto-generated id for inserted entity of type " + this.getEntityName() + ".");
                }
            }
        } catch (Exception ex) {
            System.err.println("Failed to save entity of type " + this.getEntityName() + ". (" + ex.getClass().getName() + ": " + ex.getMessage() + ")");
        }
    }
    ...
    public void delete(T entity) {
        if (entity.getId() != null) {
            try {
                ...
                System.err.println("Failed to delete entity of type " + this.getEntityName() + ". (" + ex.getClass().getName() + ": " + ex.getMessage() + ")");
            }
        }
    }
     ...
     *
     * <p>Also takes care of internal caching to prevent the creation of multiple objects for
     * the same entity.
     *
    ...
    protected T getEntityFromResultSet(ResultSet result) {
        ...
        try {
            ...
            System.err.println("Failed to retrieve ID for entity of type " + this.getEntityName() + ". (" + ex.getClass().getName() + ": " + ex.getMessage() + ")");
        }
        return entity;
    }
     ...
     *
     * <p>Overwrite this method only if you really need to change the name of the ID column.
     *
     ...
     *
     * <p>This statement contains a single placeholder for the entity's ID.
     *
     ...
     *
     * <p>This statement contains no placeholders at all.
     *
     * @return SQL statement
     */
     ...
     *
     * <p>This statement contains n placeholders, one for each field of the entity. The order follows
     ...
     *
     * <p>This statement contains n+1 placeholders, where 1..n are for the entity's fields. The order follows
     ...
     *
     * <p>Only overwrite this method, if you don't want the name to be the same as the entity's.
     *
     ...
     *
     * <p>If you need to change the name of the database table name, overwrite {@link DataAccessObject#getTableName()}.
     *
     ...
     *
     * <p>The order is order of these fields is important and you will need it for the implementation
     ...
     *
     * <p>This will be used by some of the standard SQL statements.
     *
     ...
     * entity object.
     *
     * <p>The ResultSet has already been iterated to the correct row. Retrieve all of your fields
     * from the result and place the values inside your newly created entity object.
     *
     * <p>Note: You don't have to set the ID yourself, the DAO will take care of it.
     *
     ...
     *
     * <p>Retrieve all values from your entity and assign them to the statement's parameters
     * in the same order as you returned the field values in {@link DataAccessObject#getFieldList}.
     *
     * <p>The parameters are counted from 1..n, where n is the total number of fields.
     *
     ...
     *
     * <p>If your entity has relationships with other entities, use this method to make sure they
     * are stored to the database before a foreign key constraint can fail.
     *
     * <p>You might have to adjust your constructor to make sure you have access to a
